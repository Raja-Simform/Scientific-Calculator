...
document.addEventListener("click", function (event) {
  ...
  if (
    trigDropdown &&
    event.target !== trigButton &&
    !trigButton.contains(event.target) &&
    !trigDropdown.contains(event.target)
  ) {
    trigDropdown.remove();
  }
  if (
    funcDropdown &&
    event.target !== funcButton &&
    !funcButton.contains(event.target) &&
    !funcDropdown.contains(event.target)
  ) {
    funcDropdown.remove();
  }
});
