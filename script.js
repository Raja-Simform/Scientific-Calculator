...
window.onclick = function(event) {
  if(!event.target.closest('.trigno-dropbtn')){
    if(document.getElementById("trigno-myDropdown").classList.contains('show-trigno')){
      document.getElementById("trigno-myDropdown").classList.remove('show-trigno');
    }
  }
  if(!event.target.closest('.func-dropbtn')){
    if(document.getElementById("func-myDropdown").classList.contains('show-func')){
